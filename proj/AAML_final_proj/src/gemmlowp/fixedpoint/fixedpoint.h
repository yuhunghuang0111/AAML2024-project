...
FixedPoint<tRawType, 0> exp_on_interval_between_negative_one_quarter_and_0_excl(
  ...
  F x4_over_24_plus_x3_over_6_plus_x2_over_2 =
      ...
      printf("x = %d\n",static_cast<int>(x.raw()));
      printf("x4_over_24_plus_x3_over_6_plus_x2_over_2 = %d\n",static_cast<int>(x4_over_24_plus_x3_over_6_plus_x2_over_2.raw()));
      printf("AddSaturatingIf16Bit = %d\n",static_cast<int>(AddSaturatingIf16Bit(constant_term,constant_term * (x + x4_over_24_plus_x3_over_6_plus_x2_over_2)).raw()));
...
FixedPoint<tRawType, 0> exp_on_negative_values(
  ...
  // printf("\na = %d\n",static_cast<int>(a.raw()));
  // printf("a_mod_quarter_minus_one_quarter = %d\n",static_cast<int>(a_mod_quarter_minus_one_quarter.raw()));
  ...
  // ===============================================================================================
   ...
   typedef FixedPoint<tRawType, 0> F;
   const F constant_term = GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(F,1895147668 , std::exp(-1.0 / 8.0)); // 1895147668
  tRawType cfu_result_raw = cfu_op0(1, 0, 0);
  
  F aa = Rescale<0>(a_mod_quarter_minus_one_quarter) + F::template ConstantPOT<-3>();
  cfu_result_raw = (cfu_op0(10, aa, 0));
  tRawType remainder = (a_mod_quarter_minus_one_quarter - a).raw();
  ResultF result_temp = ResultF::FromRaw(cfu_result_raw);
  ResultF result2 = AddSaturatingIf16Bit(constant_term, constant_term * (result_temp));
...
#define GEMMLOWP_EXP_BARREL_SHIFTER(Exponent, FixedPointMultiplier)         \
  if (kIntegerBits > Exponent) {                                            \
    ...
    static constexpr int kShiftAmount =                                     \
        ...
        ResultF temp = ResultF::FromRaw(cfu_op0(12, result2, kMultiplier)); \
        result2 = SelectUsingMask(                                          \
...
#undef GEMMLOWP_EXP_BARREL_SHIFTER
  ...
  if (kIntegerBits > 5) {
    ...
    result2 = SelectUsingMask(MaskIfLessThan(a, clamp), ResultF::Zero(), result2);
  }

  result2 = SelectUsingMask(MaskIfZero(a), ResultF::One(), result2);
...
perf_enable_counter(4);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    x = x + Rescale<2>(x * one_minus_half_denominator_times_x);
    */
     F2 result_t = F2::FromRaw(cfu_op0(11, x, half_denominator));
     x = x + Rescale<2>(result_t);
  }
