...
alignas(8) uint8_t tensor_arena[kTensorArenaSize] __attribute__((section(".arena")));
#else
alignas(8) static uint8_t tensor_arena[kTensorArenaSize];
#endif
}  // anonymous namespace
...
TfLiteTensor* tflite_get_input_tensor(const int input_id){
  return interpreter->input(input_id);
}

TfLiteTensor* tflite_get_output_tensor(const int output_id){
  return interpreter->output(output_id);
}
